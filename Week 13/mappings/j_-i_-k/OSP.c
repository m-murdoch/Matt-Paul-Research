...
void OSP(long N, int* M, int** C){
	...
	#define S2(i,j,k) {int __temp__ = ((C(-j+N,i-k))+(C(i-k+1,i)))+(((M(-j+N-1))*(M(i-k)))*(M(i))); Main(-j+N,i) = __min_int(Main(-j+N,i),__temp__); }
	{
		//Domain
		//{i,j,i2|i2==1 && i+j==N && N>=2 && N>=i && i>=1}
		//{i,j,i2|N+i2==i+j+1 && i+j>=N+1 && N>=2 && N>=j+1 && N>=i}
		//{i,j,i2|i2==0 && N>=j+1 && i+j>=N+1 && N>=i && N>=2}
		//{i,j,k|i+j>=N+k && k>=1 && N>=2 && N+k>=i && j>=-1 && N>=j+1 && i>=0 && N>=i && i>=k && i+j>=N+1}
		int c1,c2,c3;
		S0((1),(N-1),(1));
		for(c1=2;c1 <= N;c1+=1)
		 {
