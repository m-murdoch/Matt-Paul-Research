...
void OSP(long N, int* M, int** C){
	...
	#define S2(i,j,k) {int __temp__ = ((C(-i+j,j-k))+(C(j-k+1,j)))+(((M(-i+j-1))*(M(j-k)))*(M(j))); Main(-i+j,j) = __min_int(Main(-i+j,j),__temp__); }
	{
		//Domain
		//{i,j,i2|i2==1 && i==0 && N>=2 && N>=j && j>=1}
		//{i,j,i2|i2==i+1 && i>=1 && N>=2 && j>=i+1 && N>=j}
		//{i,j,i2|i2==0 && j>=i+1 && i>=1 && N>=j && N>=2}
		//{i,j,k|i>=k && k>=1 && N>=2 && N+k>=j && N+i>=j-1 && j>=i+1 && j>=0 && N>=j && j>=k && i>=1}
		int c1,c2,c3;
		for(c2=1;c2 <= N;c2+=1)
		 {
		 	S0((0),(c2),(1));
